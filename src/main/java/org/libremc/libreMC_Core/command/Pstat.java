...
public class Pstat implements CommandExecutor {

    void parseResultSet(CommandSender sender, ResultSet set) throws SQLException {
        while (set.next()) {
            Bukkit.getServer().getLogger().info("Iteration");
            ...
            Date expire_date = new Date(date_of_expiry - date_issued);
            long diff_time = expire_date.getTime();
            ...
            if(type.compareTo(Punishment.PunishmentType.BANNED.name()) == 0){
                sender.sendMessage(date_string + ": " + ChatColor.RED + ChatColor.BOLD  + "BANNED" + ChatColor.RESET + "- Reason: " + ChatColor.ITALIC +reason + ChatColor.RESET + "\nLength: " + expire_string + "\n");
            }else if(type.compareTo(Punishment.PunishmentType.WARNED.name()) == 0){
                sender.sendMessage(date_string + ": " + ChatColor.GOLD + ChatColor.BOLD  + "WARNED" + ChatColor.RESET + "- Reason: " + ChatColor.ITALIC +reason + ChatColor.RESET + "\n");
            }else if(type.compareTo(Punishment.PunishmentType.MUTED.name()) == 0){
                sender.sendMessage(date_string + ": " + ChatColor.BLUE + ChatColor.BOLD  + "MUTED" + ChatColor.RESET + "- Reason: " + ChatColor.ITALIC +reason + ChatColor.RESET + "\nLength: " + expire_string + "\n");
            }else{
